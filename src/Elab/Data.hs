...
elabData info syn doc argDocs fc opts (PDatadecl n nfc t_in dcons)
    = do let codata = Codata `elem` opts
         ...
         (runIO . putStrLn) ("start debugging info from elabData")
         (runIO . putStrLn . groom) (PDatadecl n nfc t_in dcons)
         (runIO . putStrLn . groom) (Data n ttag cty unique cons)
         (runIO . putStrLn) ("end debugging info from elabData")
